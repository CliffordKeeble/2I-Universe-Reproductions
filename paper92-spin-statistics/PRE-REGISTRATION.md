...
**Status:** LOCKED and executed. This is a verbatim copy of the protocol frozen
in `bootstrap-universe/deuteron-spin-statistics/PRE_REGISTRATION.md` at commit
**810e733** (the timestamped lock witness), blessed by CinC on 5 June 2026 with
the neutrino-ejection mechanism and the permission≠requirement hard rule added
before the lock. Compute followed the lock; see `findings.md` for the verdict
(OUTCOME 1, conditional). The protocol body below is unchanged from the lock.
